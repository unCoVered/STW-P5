...
public class Dia extends Element
{
	 ...
	 * @param dia
	 */
	public Dia(parser.datos.Dia dia)
	{
		super(common.CodeNames.DIA_ATR);
		this.dia = dia;
		this.setAttribute(common.CodeNames.FECHA_ATR, dia.getFecha());
		...
		this.probPrecipitacion = new Element(common.CodeNames.PROB_PRECIPITACION);
		this.cotaNieveProv = new Element(common.CodeNames.COTA_NIEVE_PROV);
		this.estadoCielo = new Element(common.CodeNames.ESTADO_CIELO);
		this.viento = new Element(common.CodeNames.VIENTO);
		this.rachaMax = new Element(common.CodeNames.RACHA_MAX);
		this.temperatura = new Element(common.CodeNames.TEMPERATURA);
		this.sensacionTermica = new Element(common.CodeNames.SENS_TERMICA);
		this.humedadRelativa = new Element(common.CodeNames.HUMEDAD_RELATIVA);
		this.uvMax = new Element(common.CodeNames.UV_MAX);
	...
	private void rellenarPeriodos(Element element, List<String> periodos)
	{
		...
		{
			element.setAttribute(common.CodeNames.PERIODO_ATR, periodo);
			...
			case "viento":
			{
				try
				{
					...
					if (datosViento != null)
					{
						this.viento.addContent(new Element(common.CodeNames.DIRECCION_ATR).setText(datosViento[0]));
						this.viento.addContent(new Element(common.CodeNames.VELOCIDAD_ATR).setText(datosViento[1]));
					}
				} catch (Exception ex)
				{
			...
			default:
				System.out.println(common.CodeNames.NO_IMPLEMENTADO_ERROR);
			}
		}
	}
	...
	private void rellenaObjetos(Element objeto, List<String> horas)
	{
		...
		case "temperatura":
		{
			try
			{
				...
				{
					this.temperatura.addContent(new Element(common.CodeNames.MAXIMA_ATR)
							.setText(Integer.toString(dia.getTemperatura().getMaxima())));
					this.temperatura.addContent(new Element(common.CodeNames.MINIMA_ATR)
		...
		case "sens_termica":
		{
			try
			{
				...
				{
					this.sensacionTermica.addContent(new Element(common.CodeNames.MAXIMA_ATR)
							.setText(Integer.toString(dia.getSensacionTermica().getMaxima())));
					this.sensacionTermica.addContent(new Element(common.CodeNames.MINIMA_ATR)
		...
		case "humedad_relativa":
		{
			try
			{
				...
				{
					this.humedadRelativa.addContent(new Element(common.CodeNames.MAXIMA_ATR)
							.setText(Integer.toString(dia.getHumedadRelativa().getMaxima())));
					this.humedadRelativa.addContent(new Element(common.CodeNames.MINIMA_ATR)
